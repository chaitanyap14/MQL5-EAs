...
void OBbull(ENUM_TIMEFRAMES TF, MqlRates& bars[], int a)
{
   ...
   for(i; i<=ArraySize(bars)+1; i++){
      if (bars[i].close <= bars[i].open || bars[i].low <= bars[i+1].low || (bars[i].high <= bars[i+1].high && bars[i].low >= bars[i+1].low)){
         insideBar(TF, bars, i);
...
void OBbear(ENUM_TIMEFRAMES TF, MqlRates& bars[], int a)
{
   ...
   int i = a+1;
   for(i; i<=ArraySize(bars)+1; i++){
      if(bars[i].close >= bars[i].open || bars[i].high >= bars[i+1].high || (bars[i].high <= bars[i+1].high && bars[i].low >= bars[i+1].low)){
         insideBar(TF, bars, i);
...
bool checkdt(){
   ...
   if(dt.day <= 15 && ( (hourmin >= 1130 && hourmin <= 130) )){
      return(true);
   } else {
      return(false);
   }
...
bool tradestoday(){
   TimeCurrent(dt);
   MqlDateTime fromstruct = dt;
   MqlDateTime tostruct = dt;
   fromstruct.hour = 11;
   fromstruct.min = 30;
   fromstruct.sec = 0;
   tostruct.hour = 15;
   ...
   HistorySelect(StructToTime(fromstruct), StructToTime(tostruct));
...
//            }
         ...
         if(trendbuffer[0] == -1.0 && checkdt() && checkorder()) {
            ...
            //Error handling
            if(!trade.BuyLimit(NormalizeDouble(calcvol(), 2), OBhigh, _Symbol, calcsl()-_Point, calctp(), ORDER_TIME_SPECIFIED, expiry, NULL)){
               trade.BuyStop(NormalizeDouble(calcvol(), 2), OBhigh, _Symbol, calcsl()-_Point, calctp(), ORDER_TIME_SPECIFIED, expiry, NULL);
            }
            //trade.SellStop(NormalizeDouble(calcvol(), 2), calcsl(), _Symbol, calcsl()+hedgesl(), calcsl()-hedgetp(), ORDER_TIME_SPECIFIED, expiry, NULL);
            ...
            //Error handling
            if(!trade.SellLimit(NormalizeDouble(calcvol(), 2), OBlow, _Symbol, calcsl()+_Point, calctp(), ORDER_TIME_SPECIFIED, expiry, NULL)){
               trade.SellStop(NormalizeDouble(calcvol(), 2), OBlow, _Symbol, calcsl()+_Point, calctp(), ORDER_TIME_SPECIFIED, expiry, NULL);
            }
            //trade.BuyStop(NormalizeDouble(calcvol(), 2), calcsl(), _Symbol, calcsl()-hedgesl(), calcsl()+hedgetp(), ORDER_TIME_SPECIFIED, expiry, NULL);
